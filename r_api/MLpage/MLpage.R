...
ui <- fixedPage(
  fixedRow(
    column(
      width = 6, plotlyOutput("donut_chart")),
    column(
      width = 6, plotlyOutput("debtChart"))
    )
  )
...
server <- function(input, output) {
  ...
  create_donut_chart <- function(data) {
    plot_ly(data, labels = ~company, values = ~market_share, type = 'pie', 
            hole = 0.4, textinfo = 'none', hoverinfo = 'label+percent', 
            marker = list(colors = c('#f3f3f3', '#a9c0e8', '#f3f3f3', '#f3f3f3','#f3f3f3', '#f3f3f3','#f3f3f3', '#f3f3f3'),
                          line = list(width = 10, color = '#fff')  # Add gaps by defining a white border between sections
  ...
  company_data <- data.frame(
    company = c("Biocon Limited", "Panacea Biotec Limited", "Hester Biosciences Limited", "Zenotech Laboratories Limited", "Nirman Agri Genetics Limited",'Bharat Immunologicals (BIBCOL)','Transgene Biotek Limited','Hindustan Bio Sciences Limited	'),
    market_share = c(89, 4, 2, 0.25, 0.38,0.28,0,0.01)
  )
  
  # Create the donut chart plot
  output$donut_chart <- renderPlotly({
    create_donut_chart(company_data)
  })
}
